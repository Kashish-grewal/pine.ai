...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f3f4f6;margin:0;padding:20px;">
  <div style="max-width:600px;margin:0 auto;">
    ...
    <div style="background:#fff;border-radius:0 0 12px 12px;padding:28px;">
      ...
      <div style="margin-bottom:24px;">
        <div style="font-size:16px;font-weight:600;border-bottom:2px solid #e5e7eb;padding-bottom:8px;margin-bottom:12px;">❓ Open Questions</div>
        <ul style="margin:0;padding-left:20px;">
          ${s.open_questions.map(q => `<li style="margin-bottom:6px;color:#374151;">${q}</li>`).join('')}
        </ul>
      </div>` : ''}
      ...
      <div style="margin-bottom:24px;">
        <div style="font-size:16px;font-weight:600;border-bottom:2px solid #e5e7eb;padding-bottom:8px;margin-bottom:12px;">📅 Upcoming Deadlines</div>
        ${s.deadlines.map(d => `
          <div style="background:#eff6ff;border-left:4px solid #3b82f6;border-radius:6px;padding:10px 12px;margin-bottom:8px;">
            <strong style="color:#1d4ed8;">${d.due || 'TBD'}</strong> — ${d.item || ''}
          </div>
        `).join('')}
      </div>` : ''}
      ...
      <div style="background:#f9fafb;border-radius:8px;padding:16px;font-size:12px;color:#9ca3af;text-align:center;">
        Generated by <strong>Pine.AI</strong> · Built by Kashish
      </div>
    </div>
  </div>
</body>
...
const sendSummaryEmail = async (options) => {
  ...
  if (!transporter) {
    throw new Error('Email sending is disabled — set GMAIL_USER + GMAIL_APP_PASSWORD or RESEND_API_KEY');
  }
  ...
  try {
    ...
    console.log(`[Email] Sent summary to ${toEmail} via ${transportType}`);
    return response;
  } catch (err) {
